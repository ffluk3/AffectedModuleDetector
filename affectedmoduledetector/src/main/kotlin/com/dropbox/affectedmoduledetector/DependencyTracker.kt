...
class DependencyTracker(rootProject: Project, logger: Logger?) : Serializable {
    ...
    // Eagerly computed at construction so no live Project reference is retained in a
    // serializable field (required for Gradle configuration cache compatibility).
    private val dependentList: Map<ProjectPath, Set<ProjectPath>> = buildDependentList(rootProject, logger)

    companion object {
        private fun buildDependentList(
            rootProject: Project,
            logger: Logger?
        ): Map<ProjectPath, Set<ProjectPath>> {
